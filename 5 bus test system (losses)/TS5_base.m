...
base_2(counter, 2) = DeltaLoad(counter);
base_2(counter, 3) = TotalLoss(counter);
base_2(counter, 4) = DeltaLoss(counter);
base_2(counter, 5) = Gen1values(counter);
base_2(counter, 6) = Gen2values(counter);
base_2(counter, 7) = Gen3values(counter);
base_2(counter, 8) = TotalGeneration(counter);
...
%being tested.
save base_2.mat base_2;

%Optional figure position
...
title('DeltaLoad vs. DeltaLoss (Base - Bus 2)') %Change to the current bus number
